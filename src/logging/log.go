...
func SetID(rid string) {
	id = rid
	exepath, err := os.Executable() //获取当前可执行文件的绝对路径
	if err != nil {
		log.Fatalln("[WriteLog Error] cannot get absolute path of executable.", err)
	}

	//p1 := path.Dir(exepath) //获取 exepath 路径的父目录，去尾
	homepath = path.Dir(exepath) //获取 p1 路径的父目录，去两层尾
}
...
func PrintLog(verbose bool, logType TypeOfLog, msg string) {
	...
	if logOpt == 0 {
		...
		case BreakdownLog:
			if BreakLogger == nil {
				fpath = fmt.Sprintf(homepath+"/var/log/%s/", id)
				fileName = fmt.Sprintf("%s_Breakdown.log", time.Now().Format("20060102"))
				if !IsExist(fpath) {
					err := CreateDir(fpath)
					if err != nil {
						log.Printf("[WriteLog Error] create log file error!", err)
				...
				fmt.Println("open ", fpath+fileName)
				Break_LogFile, err = os.OpenFile(fpath+fileName, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
				if err != nil {
					log.Printf("[WriteLog Error] open log file error!", err)
					return
				}
				BreakLogger = log.New(Break_LogFile, "", log.LstdFlags)
			}
			BreakLogger.Println(msg)
		case InfoLog:
			if Info_LogFile == nil {
				fpath = fmt.Sprintf(homepath+"/var/log/%s/", id)
