...
func StartRBC(instanceid int, input []byte) {
	...
	//p := fmt.Sprintf("[%v] Starting RBC2 for epoch %v", instanceid, epoch.Get())
	//logging.PrintLog(verbose, logging.NormalLog, p)

	msg := message.ReplicaMessage{
		Mtype:    message.RBC2_SEND,
		Instance: instanceid,
		Source:   id,
		TS:       utils.MakeTimestamp(),
		Payload:  input,
	...
	sender.MACBroadcast(msgbyte, message.RBC2)
}
...
func HandleRBCMsg(inputMsg []byte) {
	...
	switch mtype {
	case message.RBC2_SEND:
		HandleSend(content)
	case message.RBC2_ECHO:
		HandleEcho(content)
	case message.RBC2_READY:
		HandleReady(content)
	default:
