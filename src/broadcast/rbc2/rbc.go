...
func HandleSend(m message.ReplicaMessage) { // 处理Send消息
	...
	//p := fmt.Sprintf("[%v] rbc2 Handling send message from node %v", m.Instance, m.Source)
	//logging.PrintLog(verbose, logging.NormalLog, p)
	instancestatus.Insert(m.Instance, int(STATUS_SEND)) // 标记为SEND阶段

	msg := m
	msg.Source = id
	msg.Mtype = message.RBC2_ECHO
	...
	sender.MACBroadcast(msgbyte, message.RBC2) // 广播给所有人
...
func HandleEcho(m message.ReplicaMessage) { // 处理Echo消息
	...
	//p := fmt.Sprintf("[%v] rbc2 Handling echo message from node %v", m.Instance, m.Source)
	//logging.PrintLog(verbose, logging.NormalLog, p)
...
func SendReady(m message.ReplicaMessage) { // 发送Ready
	...
	if stat == int(STATUS_SEND) { // 处于SEND状态，接收过Send消息
		...
		elock.Unlock()
		//p := fmt.Sprintf(" rbc2 Sending ready for instance id %v", m.Instance)
		//logging.PrintLog(verbose, logging.NormalLog, p)

		msg := m
		msg.Source = id
		msg.Mtype = message.RBC2_READY
		msgbyte, err := msg.Serialize()
		if err != nil {
			log.Fatalf("failed to serialize ready message")
		}
		sender.MACBroadcast(msgbyte, message.RBC2) // 构建Ready消息序列化广播
...
func HandleReady(m message.ReplicaMessage) { // 处理Ready消息
	...
	//p := fmt.Sprintf("[%v] rbc2 Handling ready message from node %v", m.Instance, m.Source)
	//logging.PrintLog(verbose, logging.NormalLog, p)
...
func Deliver(m message.ReplicaMessage) {
	...
	if stat == int(STATUS_ECHO) { // 在ECHO阶段
		...
		//p := fmt.Sprintf("[%v] rbc2 Deliver the request epoch %v, curEpoch %v", m.Instance, m.Epoch, epoch.Get())
		//logging.PrintLog(verbose, logging.NormalLog, p)
