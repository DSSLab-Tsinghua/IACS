...
func StartRBC(instanceid int, input []byte) {
	...
	//p := fmt.Sprintf("[%v] Starting RBC3 for epoch %v", instanceid, epoch.Get())
	//logging.PrintLog(verbose, logging.NormalLog, p)

	msg := message.ReplicaMessage{
		Mtype:    message.RBC3_SEND,
		Instance: instanceid,
		Source:   id,
		TS:       utils.MakeTimestamp(),
		Payload:  input,
	...
	sender.MACBroadcast(msgbyte, message.RBC3)
}
...
func HandleRBCMsg(inputMsg []byte) {
	...
	switch mtype {
	case message.RBC3_SEND:
		HandleSend(content)
	case message.RBC3_ECHO:
		HandleEcho(content)
	case message.RBC3_READY:
		HandleReady(content)
	default:
